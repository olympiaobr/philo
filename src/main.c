...
{
	if (!check_int(argv[1], &info->nbr_philo) || !check_int(argv[2],
			&info->ms_to_starve) || !check_int(argv[3], &info->ms_to_eat)
...
{
	if (info->ms_to_starve < 60 || info->ms_to_eat < 60
		|| info->ms_to_sleep < 60)
	{
	...
	{
		ft_err("Error: Nb of times each philo must eat must be at least 0.\n");
		return (0);
	}
	return (1);
}
